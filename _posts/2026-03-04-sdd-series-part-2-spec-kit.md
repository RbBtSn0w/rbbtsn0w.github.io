...
description: "深入了解 GitHub Spec Kit：一套通过 9 个核心指令实现 Spec-Driven Development (SDD) 的实战工具包，助你消除 AI 编码中的规范漂移。"
image:
...
**GitHub Spec Kit** 是一套由 GitHub 推出的开源工具包，旨在通过 9 个标准化指令（如 `/speckit.specify` 和 `/speckit.plan`）强制推行规范驱动开发（SDD）。它通过约束大型语言模型（LLM）的行为，确保代码实现始终对齐业务意图，从根本上消除“规范漂移（Spec Drift）”。
...
## 2. GitHub Spec Kit 核心架构：5 核心 + 4 扩展

在 **GitHub Spec Kit** 的设计中，指令被严谨地划分为 **核心执行主线 (Essential Commands)** 与 **增强质量/协同 (Optional Commands)**。理解这种分层是高效应用 SDD 的前提。
...
- **动作**：向前推进至 **`/speckit.analyze`** 进行三方审计（Spec/Plan/Tasks 一致性检测）→ 无误后进入真正的 AI 自动化编码阶段。

---

> [!IMPORTANT]
> **GitHub Spec Kit 硬核约束：为何 `/implement` 会“强行停止”？**
...
**GitHub Spec Kit** 提供了一套成熟的企业级意图控制台，通过精密的 9 大指令工作流，真正实现了软件工程由“代码为核心”向“规格为核心”的跨越。这种转变不仅提升了开发效率，更为 [AI 代理 (AI Agents)](/posts/sdd-series-part-1-evolution/) 的深度协作奠定了基础。
