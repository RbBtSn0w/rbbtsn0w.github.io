...
description: "Trae 驱动的社媒自动上传项目复盘：架构设计、工程治理、测试与 CI/CD 实践。使用 AI 工具加速应用构建。"
image:
  path: /assets/img/post/2025-12/use_ai_tools_minimalist.png
  alt: "极简矢量风格的社交媒体自动化齿轮图，象征 social-auto-upload 工具的全平台分发能力"
---

> **TL;DR**: 这是一个借助强大的 AI 编程工具 Trae IDE 开发出的多平台社媒自动化发布系统（`social-auto-upload`）的全景复盘。系统支持抖音、B站、小红书等平台的一键上传发布。文章深入解析了如何利用 AI 实现架构重构、自动化测试驱动与 CI/CD 流程标准化，展现 AI 赋能全栈开发的潜力。
...
`social-auto-upload` 是一个强大的自动化工具，旨在帮助内容创作者和运营者高效地将视频内容一键发布到多个国内外主流社交媒体平台。项目基于 [Playwright 浏览器自动化](/posts/AntigravityChromeFix/) 技术，实现了对 `抖音`、`Bilibili`、`小红书`、`快手`、`视频号`、`百家号` 以及 `TikTok` 等平台的视频上传、定时发布等功能。
...
2. **模型选择**
   ...
   - 配置模型调用偏好，优化代码生成质量（相比于纯对话的 [GitHub Copilot](/posts/GithubCopilot/)，它在全工程上下文感知方面提供了不同的体验）。
...
**问题**：项目尚未建立 [CI/CD流程](/posts/ci-cd-best-practices/)，无法实现自动化测试和部署
